...
#### `.state/last-ingest-at`

**Who writes it:** `advance_ingest_cursor` (every time `last-ingest-sha` moves) and `ensure_ingest_clock` (seeds it with the current time when it is absent and never overwrites it). Both live in `hooks/lib/ingest-cursor.sh`.

**Who reads it:** `hooks/lib/ingest_gate.py`, for the batching gate's age arm. See [[ingest-gate]].

**Purpose:** Unix epoch seconds meaning "the wiki was known-current as of this moment". It pairs with `last-ingest-sha`: the SHA says *which* commit, and this file says *when*.

**What breaks if missing:** Nothing. `post-merge.sh` seeds it before running the gate. Because `.state/` is gitignored, every fresh clone starts without it and gets a fresh batching window instead of a forced ingest. A corrupt (non-numeric) value makes the gate run ingest immediately with `reason=no-timestamp`.
...
## Newer `.state/` artifacts

Several files appeared in `.state/` with the ingest v2 work and the hook refactors. All are gitignored along with the rest of `.state/`, and none are read by anything outside the run that created them — they exist for concurrency control and post-mortem.

```
<project>/llake/.state/
  post-merge.lock.d/
    owner.pid                  # PID of the holder; dir existence IS the lock
  agents/<agent-id>/
    agent.log
    orchestrator.pid           # v2 orchestrator subshell
    ingest.pid                 # legacy ingest run
    triage.pid  capture.pid    # session-capture phases
    formatter-exit             # transient: formatter's nonzero exit code
    render-stderr.tmp          # legacy ingest renderer stderr (on failure)
    planner-render.err
    fixer-render.err
    triage-render.err
    capture-render.err
    failed-bodies.txt          # v2: current bodies of pages the fixer must repair
    plan.json                  # v2: the planner's output
    fix-plan.json              # v2: the fixer's output
    applied.json  failed.json  # v2: first-pass applier results
    final-applied.json  final-failed.json   # v2: fix-pass results
    context/
      changes.json
      wiki-index.json
      diffs/<safe-name>.patch  # per-file; chunked as .001.patch etc. when large
```

Three things are worth knowing about this tree:

- **`post-merge.lock.d/` is a lock, not a log.** Its existence is the lock; `mkdir` is the atomic test-and-set. If you see it lingering, check whether `owner.pid` is alive — a lock older than an hour with a dead owner is reclaimed automatically on the next attempt. See [[post-merge-lock]].
...
- `last-ingest-sha` is written by hook shell code, never by an agent, and always together with `.state/last-ingest-at` via `advance_ingest_cursor`. A batching-gate `WAIT` holds it.
...
---
- `.state/post-merge.lock.d/` is a `mkdir`-based lock; its existence is the lock, and `owner.pid` names the holder.
